...
def train(train_loader, model, optim, epoch, opt, total_step, writer):
    ...
    for step, data in enumerate(train_loader):
        ...
        # depths = torch.zeros_like(imgs)
        # input_fea = torch.cat((imgs, depths), dim=0)
        #
        # print(next(model.parameters()).is_cuda)
        # print(imgs.is_cuda)
        # print(gts.is_cuda)
        # print(depths.is_cuda)
    ...
    if epoch % opt.epoch_save == 0:
        torch.save(model.state_dict(), save_path + str(epoch) + '_DFNet.pth')
...
if __name__ == "__main__":
    ...
    train_image_root = '{}/Imgs/'.format(opt.train_path) #{}/Images/Image/
    train_gt_root = '{}/GT/'.format(opt.train_path) #{}/GT_Objects/GT_Object/
    train_depth_root = '{}/Depth_V2/'.format(opt.train_path)
    train_loader = get_loader(train_image_root, train_gt_root, train_depth_root, batch_size=opt.batchsize, image_size=opt.trainsize, num_workers=16)
