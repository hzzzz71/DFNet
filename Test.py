...
for _data_name in ['ACOD-12K']: #ACOD-12K
    ...
    # 然后，使用 map_location 参数将模型映射到当前环境中的第一个 GPU 上

    model.load_state_dict(torch.load(opt.pth_path, map_location=lambda storage, loc: storage.cuda(0), weights_only=True))
    model.cuda()
    model.eval()

    os.makedirs(save_path, exist_ok=True)
    image_root = '{}/Imgs/'.format(data_path) #{}/Images/Image/  {}/Imgs/
    depth_root = '{}/Depth_V2/'.format(data_path)
    gt_root = '{}/GT/'.format(data_path) # {}/GT_Objects/GT_Object/    {}/GT/
